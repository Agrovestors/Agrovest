...
export async function generateMetadata(): Promise<Metadata> {
  const pageData = await client.fetch(`*[_type == "values" && slug.current == "values-production-other-cows"][0]{title, description}`);
  return {
    title: pageData?.title || "Cows | Agrovestors Farm Tech",
    description: pageData?.description || "Production of cows.",
    openGraph: {
      title: pageData?.title || "Cows - Agrovestors Farm Tech",
      description: pageData?.description || "Production of cows.",
...
export default async function Page() {
  const pageData = await client.fetch(`*[_type == "values" && slug.current == "values-production-other-cows"][0]{title, description, "mainImage": mainImage.asset->url, content}`);
  const fallback = {
    title: "Cows",
    description: "Production of cows.",
    mainImage: "/images/photorealistic-woman-organic-sustainable-garden-harvesting-produce.jpg",
    content: "Production of cows.",
  };

  const data = pageData || fallback;

  return (
    <div className="values-production-other-cows-page bg-gray-50 min-h-screen">
      <Navbar />
