...
export async function generateMetadata(): Promise<Metadata> {
  const pageData = await client.fetch(`*[_type == "values" && slug.current == "values-production-other"][0]{title, description}`);
  return {
    title: pageData?.title || "Other Animals | Agrovestors Farm Tech",
    description: pageData?.description || "Production of other animals.",
    openGraph: {
      title: pageData?.title || "Other Animals - Agrovestors Farm Tech",
      description: pageData?.description || "Production of other animals.",
...
export default async function Page() {
  const pageData = await client.fetch(`*[_type == "values" && slug.current == "values-production-other"][0]{title, description, "mainImage": mainImage.asset->url, content}`);
  const fallback = {
    title: "Other Animals",
    description: "Production of other animals.",
    mainImage: "/images/photorealistic-woman-organic-sustainable-garden-harvesting-produce.jpg",
    content: "Production of other animals.",
  };

  const data = pageData || fallback;

  return (
    <div className="values-production-other-page bg-gray-50 min-h-screen">
      <Navbar />
