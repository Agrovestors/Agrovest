...
export async function generateMetadata(): Promise<Metadata> {
  const pageData = await client.fetch(`*[_type == "values" && slug.current == "values-production-other-goat"][0]{title, description}`);
  return {
    title: pageData?.title || "Goat | Agrovestors Farm Tech",
    description: pageData?.description || "Production of goats.",
    openGraph: {
      title: pageData?.title || "Goat - Agrovestors Farm Tech",
      description: pageData?.description || "Production of goats.",
    ...
    alternates: { canonical: "https://agrovestors.com/values/production/other/goat" },
  };
}

export default async function Page() {
  const pageData = await client.fetch(`*[_type == "values" && slug.current == "values-production-other-goat"][0]{title, description, "mainImage": mainImage.asset->url, content}`);
  const fallback = {
    title: "Goat",
    description: "Production of goats.",
    mainImage: "/images/photorealistic-woman-organic-sustainable-garden-harvesting-produce.jpg",
    content: "Production of goats.",
  };

  const data = pageData || fallback;

  return (
    <div className="values-production-other-goat-page bg-gray-50 min-h-screen">
